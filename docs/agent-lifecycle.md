# The Lifecycle of an AI Agent using Code Atlas

This walkthrough illustrates how an AI Agent (like Claude or Antigravity) uses Code Atlas to solve a real-world task: **"Modify the data persistence layer to support PostgreSQL."**
...
- **Agent Action**: Calls `code_atlas.stats()`.
...
- **Agent Action**: Calls `code_atlas.find_symbol(query="Database")`.
...
- **Agent Action**: Calls `code_atlas.path_between(source="cli.app:main", target="core.persistence:SQLiteStore")`.
...
- **Agent Action**: Calls `code_atlas.impact_of_symbol(symbol="core.persistence:BaseStore")`.
...
- **Agent Action**: Calls `code_atlas.index_repo(source=".")`.
...
### Comparison: With vs. Without Code Atlas

| Feature | Without Code Atlas | With Code Atlas |
| :--- | :--- | :--- |
